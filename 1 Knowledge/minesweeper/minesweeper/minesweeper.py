...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        s = Sentence(neighbors(cell), count)
        ...
        def update_knowledge():
            ...
            while loop:
                ...
                for s in self.knowledge:
                    m = s.known_mines().copy()
                    if m:
                        loop = True
                        for c in m:
                            s.mark_mine(c)
                            self.mark_mine(c)
                    safe = s.known_safes().copy()
                    if safe:
                        loop = True
                        for c in safe:
                            s.mark_safe(c)
        ...
        def infer_new_sentence():
            for s in self.knowledge:
                for sentence in self.knowledge:
                    if s.cells.issubset(sentence.cells):
                        if s.cells != sentence.cells:
                            newsent = Sentence(sentence.cells - s.cells, sentence.count - s.count)
    ...
    def make_random_move(self):
        ...
        """
        num = self.height * self.width - len(self.moves_made) - len(self.mines)
        k = 0
        while k<num:
